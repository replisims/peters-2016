...
str(type_1_plot)
  ...
  plot_data_power <- power_table %>%
    group_by(test_type, ma_size, odds_ratio, heterogeneity) %>%
      summarise(power = mean(power)) %>% ungroup()


  plot_data_power2 <- results_h0_false %>% mutate(bias_detail = case_when(bias_type == "p" & bias_strength == "moderate"~ "p_moderate",
