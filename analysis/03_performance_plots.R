...
# Create a new variable indicating the type of publication bias
plot_data_power <- results_h0_false %>%
  mutate(bias_detail = case_when(bias_type == "p" & bias_strength == "moderate"~ "p_moderate",
...
# Power for detecting severe publication bias induced by p-value

power_plot(data = plot_data_power %>%
                    dplyr::filter(bias_detail == "p_severe"),
           file_path = "analysis/figures/power_p_severe.png")

# Power for detecting moderate publication bias induced by p-value

power_plot(data = plot_data_power %>%
                  dplyr::filter(bias_detail == "p_moderate"),
           file_path = "analysis/figures/power_p_moderate.png")

# Power for detecting moderate publication bias induced by effect size

power_plot(data = plot_data_power %>%
                  dplyr::filter(bias_detail == "es_moderate"),
           file_path = "analysis/figures/power_es_moderate.png")

# Power for detecting severe publication bias induced by effect size

power_plot(data = plot_data_power %>%
                  dplyr::filter(bias_detail == "es_severe"),
           file_path = "analysis/figures/power_es_severe.png")

# Power for detecting severe publication bias across all methods of publication bias

power_plot(data = plot_data_power %>%
                    group_by(test_type, ma_size, odds_ratio, heterogeneity) %>%
                      summarise(power = mean(power)),
           file_path = "analysis/figures/power_overall.png")
